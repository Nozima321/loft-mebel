...
function getAllChildrenProducts() {
    fetch(`${childrenProductsLink}`, {
        method: "GET",
    })
        .then(res => res.json())
        .then(data => {
...
getAllChildrenProducts();

function innerCards(data) {
    ChildrenCardsContent.innerHTML = "";
...
function getChildrenProduct(id) {
    fetch(`${childrenProductsLink}/${id}`)
        .then(res => res.json())
        .then(data => {
            changeChildrenProductStatus(data)
        })
}

function changeChildrenProductStatus(data) {
    if (data.saved) {
        data.saved = false;
    }
    else {
    ...
    fetch(`${childrenProductsLink}/${data.id}`, {
        method: "PUT",
